...
var api = {
    urlScraper: "http://192.168.1.173:8080/api/parser/"
};
var functions = {
                                    ...
                                    async.eachSeries(urls,
                                        function (url, callback2) {
                                            ...
                                            if (result.host != 'twitter.com') {
                                                 ...
                                                 traceRedirect(extendedUrl).then(function(cleanedURL) {
                                                    var siteUrl = cleanedURL.replace(/utm_[^&]+&?/g, "")
                                                    .replace(/&$/, "")
                                                    .replace(/^\?$/, "");
                                                    request(siteUrl, function(error, response, html) {
                                                        if (!error && response.statusCode === 200) {
                                                            ...
                                                            $("meta").each(function(i, element) {
                                                                if (element.attribs.property === "og:description" && ogDescription === "") {
                                                                    ogDescription = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:image" && ogImage === "") {
                                                                    ogImage = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:url" && ogUrl === "") {
                                                                    ogUrl = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:type" && ogType === "") {
                                                                    ogType = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:title" && ogTitle === "") {
                                                                    ogTitle = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:locale" && ogLocale === "") {
                                                                    ogLocale = element.attribs.content;
                                                                }
                                                                if (element.attribs.property === "og:site_name" && ogSiteName === "") {
                                                                    ogSiteName = element.attribs.content;
                                                                }
                                                                if (element.attribs.name === "Description" && description === "") {
                                                                    description = element.attribs.content;
                                                                }
                                                                if (element.attribs.name === "Keywords" && keywords === "") {
                                                                    keywords = element.attribs.content;
                                                                }
                                                                if (element.attribs.name === "author" && author === "") {
                                                                    author = element.attribs.content;
                                                                }
                                                                if (element.attribs.name === "viewport" && viewport === "") {
